...
{
    public record PayloadValidationError(string Message, Exception? Exception = null);
    public record PayloadValidationErrors(PayloadValidationError[] errors) : PayloadError(ErrorCode.PayloadValidationFailed, string.Join("; ", errors.Select(x => x.Message)));
